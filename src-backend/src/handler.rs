...
impl Handler {
    ...
    fn handle(
        ...
        match msg {
            ...
            p::IncomingMessage::Preview(info) => self.preview(&info.url, &info.id, peer_id),
            p::IncomingMessage::StopPreview(info) => self.stop_preview(&info.id, peer_id),
        }
    }
    ...
    fn remove_peer(&mut self, peer_id: &str) {
        ...
        self.pipelines.retain(|_, pipeline| {
            if pipeline.clients.contains_key(peer_id) {
                pipeline.clients.remove(peer_id);
                if pipeline.clients.len() == 0 {
                    pipeline.pipeline.set_state(gst::State::Null).expect("stop pipeline");
                    return false;
                }
            }
            true
    ...
    fn preview(&mut self, url: &str, id: &str, peer_id: &str) -> Result<(), Error> {
        info!(peer_id = %peer_id, url = %url, "preview");
        if self.pipelines.contains_key(id) {
            self.pipelines.get_mut(id).unwrap().clients.insert(peer_id.to_string(), 1);
            ...
            clients.insert(peer_id.to_string(), 1);
    ...
    fn stop_preview(&mut self, id: &str, peer_id: &str) -> Result<(), Error> {
        info!(peer_id = %peer_id, id = %id, "stop preview");
